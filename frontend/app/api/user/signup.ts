...
// Funzione di signup
const signup = async ({username, email, password, confirmPassword, tipo}: InputValues): Promise<SignupResponse> => {
    ...
    if (!username || !email || !password) {   
        return { success: false, error: "Username, email e password sono richiesti" };
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            return { success: false, error: errorData.message || "Credenziali non valide" };
        }
